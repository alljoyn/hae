...
static AJ_Status EmitPropChanged(AJ_BusAttachment* busAttachment, const char* objPath, const char* propName, const char* signature, void* val)
{
    ...
    status = MakePropChangedId(objPath, AUDIO_VOLUME_INTERFACE, &msgId);
    if (status == AJ_OK) {
...
AJ_Status AudioVolumeInterfaceOnSetProperty(AJ_Message* replyMsg, const char* objPath, void* properties, uint8_t memberIndex, void* listener, HaePropertiesChanged* propChanged)
{
    AJ_Status status = AJ_OK;

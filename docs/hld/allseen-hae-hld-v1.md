...
The _Figure 4_ describes block diagram for HAE controllee device and HAE controller device. 
The Controllee App and Controller App can be developed using the HAE Service Framework 
by different manufacturers. 
The HaeControllee or HaeController is in charge of start/stop the HAE Service of the HAE device.
The HaeControllee manages InterfaceControllees and RequestListeners.
The HaeControllee application set the local value via the InterfaceControllee and 
receive the remote request via the RequestListener. At this time, application use 
the instances of InterfaceControllee and RequestListener directly. 
The InterfaceControllee should be implemented following the HAE interface specification. 
The RequestListener that is a handler for receiving the method call of specific interface
from the controller. These InterfaceControllees and RequestListeners operate with a bus object.

HAE controller should manage interest device list. 
The HaeController can communicate with the controllee device through the ControllerInterface 
and SignalListener. Device Manager manage device list and connection status of each interest
device. Once About listener receives the About announcement from HAE devices, the
DeviceManager add device info and notify the device info connected to the application by the
DeviceListener. DeviceManager should provide a function to find interest devices using the
DeviceDescription that is one of the About custom metadata fields to represent device type
corresponding to the bus object. DeviceListener conveys each device’s connection status and
About data to the application. HAE controller application send the sync/async request via the
ControllerInterface to the controllee and receive the signal through the SignalListener. 
If controller send the async method call, the controller can receive the response via the
AsyncResponseListener. HAE controller application use those interfaces and listeners directly.
...
After joining session, the HaeController gets proxy object and creates 
the InterfaceControlller which corresponds to the InterfaceControllee of 
...
The Controller App need the ControllerInterface to call method of target interface and
the Controllee App can receive the method call via RequestListener. HAE Service Framework
provides sync/async method call. _Figure 6_ illustrates the sequence of the sync method
call. For example, if air conditioner controller wants to change the target temperature,
the Controller App can set the target temperature using the TargetTemeperatureInterfaceController
corresponding to the TargetTemperatureInterfaceControllee of the air conditioner.
The air conditioner can receive the method call of controller through the TargetTemperatureRequestListener.

![](hae_method_sequence.png)

Figure 6: Sync method call sequence

_Figure 7_ illustrates the sequence of the async method call. Async method call has
same sequence with sync method call except the receiving reply step. 
If controller requests async method call to the controllee, 
AsyncResponseListener receive the reply of controllee.

![](hae_async_method_sequence.png)

Figure 7: Async method call sequence

#### Signal Handling

Signal handling process is the other way of the method call process.
Signal handling process uses the ControlleeInterface and SignalListener.
When air conditioner change current temperature by internal logic,
the Controllee App change internal value of the CurrentTemeperatureInterfaceControllee,
This value is sent to the controller and the controller can receive 
this signal through the CurrentTemperatureSignalListener. _Figurue 8_ illustrates
this signal handling sequence.

![](hae_signal_sequence.png)

Figure 8: Signal handling sequence
...
Figure 9: Setting target temperature sequence in the air conditioner controller

#### Setting target temperature sequence of air conditioner

![](hae_airconditioner_controllee_sample.png)

Figure 10: Setting target temperature sequence of air conditioner

#### Setting operational cycle sequence in the washer controller

![](hae_washer_controller_sample.png)

Figure 11: Setting operational cycle sequence in the washer controller

#### Setting operational cycle sequence in the washer

![](hae_washer_controllee_sample.png)

Figure 12: Setting operational cycle sequence in the washer

...
This interface provides a capability to control and monitor dish washing
cycle operation.
...
It indicates the currently selected operational cycle program.
...
values that can be used to set the **OperationalCycle** property using the
...
Set the value of operational cycle.
If the _consumer_ tries to set value which is not supported (i.e. it is not in
the list of **SelectableOperationalCycles** property), then a invalid value
error should be returned.

Input arguments:

  * **cycle** --- int16 --- the operational cycle to be set

...
When a sub-unit is not plugged in the related bus object shouldn't be used by
the _consumer_ because the related interfaces are not working, so the _consumer_
itself has to be aware of it.
...
In principle this feature could be managed without the interface just updating
the bus objects structure: i.e. creating a new bus object when a sub-unit is
plugged in and deleting when it is plugged out. Anyway the implementation of
this interface lets the _consumer_ get in real time the sub-unit status by
emitting related properties change signals.
...
The **PlugInUnitStruct** is used to describes the characteristics and the status

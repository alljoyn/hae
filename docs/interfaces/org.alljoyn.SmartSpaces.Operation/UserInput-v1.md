...
interface devices from the _consumer_ to the _producer_. This interface uses
a key code in [linux input subsystem](#Linux input subsystem). This interface
supports a generic mechanism to inject user input events for various Human
interface devices such as keyboards, mice, touch screens, remote controllers,
key pads, and joy sticks, etc.
...
Inject the user input events which are defined in [linux input subsystem](#Linux input subsystem). 
for human interface devices. When occurred multiple events simultaneously, send
an array of input events. The reply is not needed. If the controllee doesn't
support event code, it just ignores the event code without returning error.

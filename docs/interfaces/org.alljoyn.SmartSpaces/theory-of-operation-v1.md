...
|Dish Washer         | 15 | Device that cleans dishes and eating utensils.| Operation.OnOffStatus, Operation.OnControl, Operation.OffControl, Operation.CycleControl, Operation.DishWashingCycle |
...
|FoodProbe           | 20 | Device used for checking the preparation of foods (typically the meat) to ensure that the appropriate and safe internal temperature has been achieved. Some probes are inserted into the food, other are just placed close to it. | Environment.TargetTemperature, Operation.PlugInUnits |

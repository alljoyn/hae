...
start with "org.alljoyn.Hae.Device". Each device interface has minimum
mandatory requirements on shared interfaces. "Theory of Operation" section
of each device interface defintion describes the list of minimum required shared
interfaces.

| Device Type | Description                                        |
|-------------|----------------------------------------------------|
|Container|Container device that can be a container including two or more standard devices interfaces.|
|Fridge|Device that stores food at temperatures which are a few degrees above the freezing point of water.|
...
|Washer|Device that washes laundry such as clothing and sheets|
|Dryer|Device that removes moisture from a load of clothing and other textiles usually shortly after they are washed in a washer.|
|Washer Dryer|Device that executes the operations of both a washer and a dryer in a single cavity.|
...
|TV|Television set with integrated capabilities that offers more advanced computing ability and connectivity.|

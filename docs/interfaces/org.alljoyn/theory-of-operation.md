...
implemented using the Controller API of the HAE Service Framework. Figure 2
...
chilldren of children device shall be prohibited. In order to comprise of two
...
start with "org.alljoyn.Hae.Device". Table below shows the standard device types
for the HAE Service Framework and their minimum required shared interfaces.

| Device Type | Description                                        | Minimum Mandatory Requirements on Shared Interfaces|
|-------------|----------------------------------------------------|----------------------------------------------------|
|Container|Container device that can be a container including two or more standard devices interfaces.|None|
|Fridge|Device that stores food at temperatures which are a few degrees above the freezing point of water.|DoorStatus|
|Freezer|Device that stores food at temperatures which are a few degrees below the freezing point of water (typically about -18 °C), so the food itself is safe indefinitely.|DoorStatus |
|Ice Maker|Device that makes ice. It exists as a sub-unit inside a refrigerator container or as a standalone-device.| None |
|Air Conditioner|Device that alters the properties of air (primarily temperature and humidity) to more comfortable conditions|OperationalControl, TargetTemperature, RepresentedTemperature |
|Humidifier|Device that increases air humidity (moisture) in a single room or an entire building.|OperationalControl, MoistureOutputRate |
|Dehumidifier|Device that decreases air humidity (moisture) in a single room or an entire building.|OperationalControl, TargetHumidity,  RepresentedHumidity |
|Air Purifier|Device that has a filter that removes dust from the air.|OperationalControl|
|Electric Fan|Traditional style fan usually with blades visible from outside.|OperationalControl|
|Air Quality Monitor|Device that checks and reports the air quality.|RepresentedAirQualityLevel|
...
|Washer|Device that washes laundry such as clothing and sheets|OperationalControl|
|Dryer|Device that removes moisture from a load of clothing and other textiles usually shortly after they are washed in a washer.|OperationalControl|
|Washer Dryer|Device that executes the operations of both a washer and a dryer in a single cavity.|OperationalControl|
|Dish Washer|Device that clean dishes and eating utensils.|OperationalControl|
|Oven|Device that roasts and heats food in a complete stove|OperationalControl, OperationalTime, TargetTemperature, RepresentedTemperature|
...
|TV|Television set with integrated capabilities that offers more advanced computing ability and connectivity.|OperationalControl, VolumeControl |
|Cooker Hood|Home appliance that usually hangs above the stove or cooktop and it is used to removes airborne grease, combustion products, fumes, smoke, odours, heat, and steam from the air by evacuation of the air and filtration|FilterStatus, WindStrength|
|Cooktop|Home appliance with a flat framework used to place pots to be heated so the food inside is cooked|None|
|SetTopBox|Home appliance can receive audio and visual contents such as broadcasting program and network streaming and convert them for showing on a display device such as TV. |OperationalControl|
...
Figure 4: Device model for refrigerator

A refrigerator consists of three children devices: Freezer, Fridge and Ice Maker.
The container device interface can be used to represent a refrigerator like
Figure 4. The refrigerator allows to change its target temperature and display
the current temperature for its freezer and fridge, respectively. In addition
to that, it has an additional capability of ice making and this capability can

...
**Figure:** Operational Control state chart
...
internal operational state, then an appropriate error shall be returned.
...
Errors raised by this method:
  ...
  * org.alljoyn.Hae.Error.NotAcceptableDueToInternalState --- when the command
    value is not accepted by the _producer_, because it is in a state which
    doesn't allow the related transition
  * org.alljoyn.Hae.Error.RemoteControlDisabled --- when the remote control is
    disabled

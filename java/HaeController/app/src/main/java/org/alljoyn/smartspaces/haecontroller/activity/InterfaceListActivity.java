...
public class InterfaceListActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        aboutObjectDescriptionAdapter = new LevelListAdapter(this);
        ...
        interfaceListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                LevelListAdapter.Item item = aboutObjectDescriptionAdapter.getItem(position);
                if (item.level == 0) {
                ...
                if (device != null) {
                    for (AboutObjectDescription desc : device.getAboutObjectDescriptions()) {
                        if (desc.path.toUpperCase().contains("HAE")) {
                            InterfaceListActivity.this.aboutObjectDescriptionAdapter.add(0, 0, desc.path, desc.path);
                            for (String intf : desc.interfaces) {
                                InterfaceListActivity.this.aboutObjectDescriptionAdapter.add(1, 0, intf.substring(intf.lastIndexOf('.') + 1), intf);
                            }
                        }
                    }
                }

...
void CycleControlListener::OperationalStatePropertyChanged(const qcc::String& objectPath, const uint8_t state)
{
    cout << __func__ << endl;
    cout << "# path: " << objectPath << endl;
    cout << "# state: " <<(int)state <<"; state string: "<< OPERATIONAL_STATE_STRINGS[state] << endl;
}


void CycleControlListener::GetOperationalStatePropertyCallback(QStatus status, const qcc::String& objectPath, const uint8_t state, void* context)
{
    cout << __func__ << endl;
...
void CycleControlListener::GetSupportedStatesPropertyCallbalck(QStatus status, const qcc::String& objectPath, SupportedOperationalStates states, void* context)
{
    cout << __func__ << endl;
    cout << "# status: " << QCC_StatusText(status) << endl;
    cout << "# path: " << objectPath << endl;
    ...
    {
        cout << "# state: " << (int)states[i] << "; state string: " << OPERATIONAL_STATE_STRINGS[states[i]] << endl;
    }
}

void CycleControlListener::GetSupportedCommandsPropertyCallbalck(QStatus status, const qcc::String& objectPath, SupportedOperationalCommands commands, void* context)
{
    cout << __func__ << endl;
    cout << "# status: " << QCC_StatusText(status) << endl;
    cout << "# path: " << objectPath << endl;
    ...
    {
        cout << "# command: " << (int)commands[i] << "; command string: " << OPERATIONAL_COMMAND_STRINGS[commands[i]] << endl;
    }
}

void CycleControlListener::SupportedOperationalStatesProperyChanged(const qcc::String& objectPath, SupportedOperationalStates states)
{
    cout << __func__ << endl;
    cout << "# path: " << objectPath << endl;
    cout << "# supported states: " << endl;
...
void CycleControlListener::SupportedOperationalCommandsProperyChanged(const qcc::String& objectPath, SupportedOperationalCommands commands)
{
    cout << __func__ << endl;
    cout << "# path: " << objectPath << endl;
    cout << "# supported commands: " << endl;
...
void CycleControlCommands::OnCmdExecuteCommand(Commands * commands, std::string& cmd)
{
    ...
    if ( command >=0 && command <=3 ) {
        intfController->ExecuteCommand((uint8_t)command);
    } else {

...
/*void ClosedStatusListener::SetIsClosedPropertyCallback(QStatus status, const qcc::String& objectPath, void* context)
{
    cout << __func__ << endl;
    cout << "status: " << QCC_StatusText(status) << endl;
    cout << "path: " << objectPath << endl;
...
void ClosedStatusCommands::Init()
{
    ...
    //RegisterCommand(&ClosedStatusCommands::OnCmdSetIsClosed, "scs", "set isClosed (use 'scs <0/1>'");
    PrintCommands();
}

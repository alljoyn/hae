...
void LaundryControllee::OnCmdSetState(Commands* commands, std::string& cmd)
{
    ...
    if (static_cast<LaundryControllee*>(sample)->m_cycleControlIntfControllee) {
        static_cast<LaundryControllee*>(sample)->m_cycleControlIntfControllee->SetOperationalState(state);
    }
}
...
void LaundryControllee::SetInitialProperty()
{
	...
	SupportedOperationalCommands comm;
	comm.push_back(OPERATIONAL_COMMAND_STOP);
	comm.push_back(OPERATIONAL_COMMAND_PAUSE);
	comm.push_back(OPERATIONAL_COMMAND_RESUME);
	m_cycleControlIntfControllee->SetSupportedCommands(comm);

	SupportedOperationalStates states;
	states.push_back(OPERATIONAL_STATE_IDLE);
	states.push_back(OPERATIONAL_STATE_PAUSED);
	states.push_back(OPERATIONAL_STATE_READY_TO_START);
	states.push_back(OPERATIONAL_STATE_END_OF_CYCLE);
	states.push_back(OPERATIONAL_STATE_DELAYED_START);
	states.push_back(OPERATIONAL_STATE_WORKING);

	m_cycleControlIntfControllee->SetSupportedStates(states);


	uint8_t maxLvl = m_soilLevelIntfControllee->GetMaxLevel();
	std::vector<uint8_t> selectableLevels(maxLvl);
	...
	maxLvl = m_waterLevelIntfControllee->GetMaxLevel();
    std::vector<uint8_t> selectableLevelsSpin(maxLvl);
    for(int i = 0; i < maxLvl; i++)
        selectableLevelsSpin.push_back(i);

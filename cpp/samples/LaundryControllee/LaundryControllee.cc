...
#include "../BaseSample/VendorDefinedInterfaces/VendorDefinedIntfControllee.h"
...
class LaundryControllee : public ControlleeSample
{
  private:
    ...
    VendorDefinedIntfControllee* m_vendC;
...
LaundryControllee::LaundryControllee(BusAttachment* bus, HaeAboutData* aboutData)
  : ControlleeSample(bus, aboutData),m_onOffStatusIntfControllee(NULL), m_cycleControlIntfControllee(NULL),m_soilLevelIntfControllee(NULL),m_spinSpeedLevelIntfControllee(NULL),
    m_waterLevelIntfControllee(NULL), m_dishWashingCyclePhaseIntfControllee(NULL), m_laundryCyclePhaseIntfControllee(NULL), m_timerIntfControllee(NULL), m_vendC(NULL)

{
    ...
    m_vendL = new VendorDefinedListener();
}
...
//	//WaterLevel interface
//	maxLvl = m_waterLevelIntfControllee->GetMaxLevel();
//    std::vector<uint8_t> selectableLevelsWater(maxLvl);
//    for(int i = 0; i < maxLvl; i++)
//        selectableLevelsWater.push_back(i);
//
//    m_waterLevelIntfControllee->SetSelectableLevels(selectableLevelsWater);

...
/*void ClosedStatusIntfControlleeImpl::ToggleIsClosed()
{
    	MsgArg val;
    	m_isClosed = !m_isClosed;
    	val.typeId = ALLJOYN_BOOLEAN;
    	val.v_bool = m_isClosed;
        ...
        if (s_retrievingActualPropertyValue)
        {
        	if(!s_prop_IsClosed.compare(propName))
        	{
                ...
                else
                {
                	...
                	//SetIsClosed(isClosed); // update the value in AudioVolumeIntfControllee.
                }
...
/*	if (!(s_prop_IsClosed.compare(propName)))
	{
		...
		bool isClosed = val.v_bool;
		status = m_interfaceListener.OnSetIsClosed(isClosed);
		if (status != ER_OK)
		{

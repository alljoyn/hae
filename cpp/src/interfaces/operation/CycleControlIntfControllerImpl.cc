...
QStatus CycleControlIntfControllerImpl::Init()
{
    ...
    if (intf) {
        ...
        if (endOfCycleEmitted) {
            status = m_busAttachment.RegisterSignalHandler(this, static_cast<SignalHandler>(&CycleControlIntfControllerImpl::EndOfCycleEmitted), endOfCycleEmitted, NULL);
            if (ER_OK != status) {
                QCC_LogError(status, ("%s: RegisterPropertiesChangedListener failed.", __func__));
            }
        } else {
    ...
    for (size_t i = 0; i < numEntries; ++i) {
        ...
        if (!s_prop_OperationalState.compare(propNameStr)) {
            if (propValue->typeId == ALLJOYN_BYTE) {
                ...
                m_interfaceListener.OperationalStatePropertyChanged(obj.GetPath(), volume);
            }
        }
...
QStatus CycleControlIntfControllerImpl::ExecuteCommand(const uint8_t command, void* context)
{

	MsgArg args[1];
	args[0].Set("y", command);
...
void CycleControlIntfControllerImpl::GetOperationalStatePropertyCB(QStatus status, ProxyBusObject* obj, const MsgArg& value, void* context)
{
    ...
    m_interfaceListener.GetOperationalStatePropertyCallback(status, obj->GetPath(), state, context);
}
...
{
	cout << "End of cycle emitted " <<endl;
}
